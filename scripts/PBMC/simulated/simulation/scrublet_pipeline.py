...
parser.add_argument("-a", "--dbl_rate", required = True, default = None , type = float, help = "Expected doublet rate")
...
scrub = scr.Scrublet(counts_matrix, expected_doublet_rate=args.dbl_rate, sim_doublet_ratio = args.sim_doublet_ratio)
...
if args.scrublet_doublet_threshold is None:
    ## check if it was able to automatically call the threshold, if not set one just so can finish rule
  if not hasattr(scrub, 'threshold_'):
    scrub.call_doublets(threshold=0.5)
