...
original_meta_file <- as.character(arguments[6,])
print(meta_file)
...
ones <- softwares
twos_df <- t(combn(softwares, 2, simplify = TRUE))
twos <- apply( twos_df , 1 , paste , collapse = "-" )
threes_df <- t(combn(softwares, 3, simplify = TRUE))
threes <- apply( threes_df , 1 , paste , collapse = "-" )
fours_df <- t(combn(softwares, 4, simplify = TRUE))
fours <- apply( fours_df , 1 , paste , collapse = "-" )
fives_df <- t(combn(softwares, 5, simplify = TRUE))
fives <- apply( fives_df , 1 , paste , collapse = "-" )
sixes_df <- t(combn(softwares, 6, simplify = TRUE))
sixes <- apply( sixes_df , 1 , paste , collapse = "-" )
sevens_df <- t(combn(softwares, 7, simplify = TRUE))
sevens <- apply( sevens_df , 1 , paste , collapse = "-" )
eights_df <-t(combn(softwares, 8, simplify = TRUE))
eights <- apply( eights_df , 1 , paste , collapse = "-" )
nines_df <- t(combn(softwares, 9, simplify = TRUE))
nines <- apply( nines_df , 1 , paste , collapse = "-" )
tens_df <- t(combn(softwares, 10, simplify = TRUE))
tens <- apply( tens_df , 1 , paste , collapse = "-" )
elevens_df <- t(combn(softwares, 11, simplify = TRUE))
elevens <- apply( elevens_df , 1 , paste , collapse = "-" )
twelves_df <- t(combn(softwares, 12, simplify = TRUE))
twelves <- apply( twelves_df , 1 , paste , collapse = "-" )
thirteens_df <- t(combn(softwares, 13, simplify = TRUE))
thirteens <- apply( thirteens_df , 1 , paste , collapse = "-" )
fourteens_df <- t(combn(softwares, 14, simplify = TRUE))
fourteens <- apply( fourteens_df , 1 , paste , collapse = "-" )
fifteens_df <- t(combn(softwares, 14, simplify = TRUE))
fifteens <- apply( fifteens_df , 1 , paste , collapse = "-" )

all_combn <- c(ones,twos,threes,fours,fives,sixes,sevens,eights,nines,tens,elevens,twelves,thirteens,fourteens,fifteens)
...
for (comparison in 1:length(all_combn_filt)){
  print(comparison)
  combination <- all_combn_filt[comparison]
  softwares <- strsplit(all_combn_filt[comparison],"-")[[1]]
  ...
  if (any(softwares %in% doublet_detection_list)){
    percent_correct[,combination] <- (rowSums(results_list_wide[,paste0(softwares[(softwares %in% doublet_detection_list)],"_DropletType")] == simulated_barcodes_ordered$DropletType, na.rm = TRUE) +
                                            rowSums(results_list_wide[,paste0(softwares[(softwares %in% demultiplexing_list)],"_DropletType")] == simulated_barcodes_ordered$DropletType & results_list_wide[,paste0(softwares[(softwares %in% demultiplexing_list)],"_Assignment")] == simulated_barcodes_ordered$Individual, na.rm = TRUE))/length(softwares)
  } else {
  ...
  if (any(softwares %in% doublet_detection_list)){
    cell_type_proportion[,combination] <- (individual_proportions[,combination]*length(softwares[softwares %in% demultiplexing_list])/length(softwares) + rowSums(results_list_wide[,paste0(softwares[(softwares %in% doublet_detection_list)], "_DropletType")] == c(cell_type[,combination]), na.rm = TRUE)/(length(softwares[(softwares %in% doublet_detection_list)]))*length(softwares[softwares %in% doublet_detection_list])/length(softwares))
  } else {
...
saveRDS(singlets, paste0(out, "/singlet_counts_per_barcode.rds"))
saveRDS(individual, paste0(out, "/most_common_individual_per_barcode.rds"))
saveRDS(individual_proportions, paste0(out, "/proportion_most_common_individual_per_barcode.rds"))
saveRDS(percent_correct, paste0(out, "/percent_correct_per_barcode.rds"))
saveRDS(cell_type, paste0(out, "/proportion_most_common_cell_type_barcode.rds"))
saveRDS(cell_type_proportion, paste0(out, "/proportion_most_common_cell_type_per_barcode.rds"))
...
saveRDS(singlets_ref, paste0(out, "/singlet_counts_per_barcode_w_ref_data.rds"))
saveRDS(individual_ref, paste0(out, "/most_common_individual_per_barcode_w_ref_data.rds"))
saveRDS(individual_proportions_ref, paste0(out, "/proportion_most_common_individual_per_barcode_w_ref_data.rds"))
