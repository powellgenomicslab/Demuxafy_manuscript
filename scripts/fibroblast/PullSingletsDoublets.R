### script to identify the common singlets and doublets across the different 
...
##### Set directories #####
dir <- "/path/to/output/fiobroblasts/" ### the outdir used in 
out <- "/path/to/output/fibroblasts/Round1Overlap/"
outdir <- "/path/to/output/fibroblasts/Round1Overlap/Figures/"
datadir <- "/path/to/10x/data/dir/" ### the directory that contains all the results from cellranger for the fibroblast pools
demuxafy_dir <- "/path/to/Demuxafy_manuscript/"
meta <- read_delim(paste0(demuxafy_dir,"files/fibroblast/fibroblast_sample_meta.tsv", delim = "\t"))
dir.create(out)
sample_list <- meta$Pool
...
##### Fix demuxlet assignments #####
results_list <- lapply(results_list, function(x){
  x$demuxlet_Assignment <- gsub(",.*", "", x$demuxlet_Assignment)
  return(x)
})
...
lapply(results_list_long, function(x) any(is.na(x$Genotype_ID)))
...
results_list_wide <- lapply(results_list_long, function(x){
  ...
  # temp <- dcast(x, colnames(x)[!(colnames(x) %in% c("Genotype_ID", "Software"))] ~ Software, value.var = "Genotype_ID")
  return(data.table(x))
})
...
if(!file.exists(paste0(outdir, "merged_seurat.rds"))){
  ##### Make Seurat Objects #####
  dirs10x <- paste0(datadir, "/", sample_list, "_V1/outs/filtered_gene_bc_matrices/Homo_sapiens_GRCh38p10/")
  ...
  seurat <- merge(seurat_list[["scFibroblast_EQTL_Sample1"]], y = seurat_list[2:length(seurat_list)],project = "Fibro_sc")



  ##### Make plots of QC metrics vs pools #####
  ## Add QC metrics
  RbGeneList <- read.delim(file = paste0(demuxafy_dir,"/files/RibosomalGeneList_GeneID_ENSG.txt")
  MtGeneList <- read.delim(file = paste0(demuxafy_dir,"/files/MtGeneList_GeneID_ENSG.txt")
  ...
  seurat$Pool <- gsub("_[A,T,G,C]+-1", "", colnames(seurat))

  saveRDS(seurat, paste0(outdir, "merged_seurat.rds"))

} else {
  seurat <- readRDS(paste0(outdir, "merged_seurat.rds"))
}

QC_metrics <- data.table(seurat@meta.data[,c("Pool", "percent.mt", "nCount_RNA", "nFeature_RNA", "Singlets", "Doublets", "Demultiplexing_Singlets", "Demultiplexing_Doublets", "DoubletDetecting_Singlets", "DoubletDetecting_Doublets")])
QC_metrics_long <- pivot_longer(QC_metrics, cols = c("percent.mt", "nCount_RNA", "nFeature_RNA"), names_to = "QC_Metric", values_to = "QC_Metric_Value")


QC_metrics_long_temp <- QC_metrics_long
QC_metrics_long_temp$Singlets <- "all"
QC_metrics_long_temp$Doublets <- "all"
QC_metrics_long_temp$Demultiplexing_Singlets <- "all"
QC_metrics_long_temp$Demultiplexing_Doublets <- "all"
QC_metrics_long_temp$DoubletDetecting_Singlets <- "all"
QC_metrics_long_temp$DoubletDetecting_Doublets <- "all"
QC_metrics_long_all_cells <- rbind(QC_metrics_long, QC_metrics_long_temp)


p_pool_qc <- ggplot(QC_metrics_long, aes(Pool, QC_Metric_Value)) +
  geom_boxplot(outlier.size = 0.5) +
  theme_bw() +
  facet_wrap(~QC_Metric, ncol = 1, scales = "free_y", strip.position = "left", labeller = as_labeller(c(percent.mt = "Mitochondrial\nPercent", nCount_RNA = "Number\nUMIs", nFeature_RNA = "Number\nGenes")))  +
  ylab(NULL) +
  theme(axis.text.x=element_blank(),
        strip.background = element_blank(),
        strip.placement = "outside") 

ggsave(p_pool_qc, filename = paste0(outdir,"Pool_QC_metrics.pdf"), width = 12, height = 12, units = "cm")
ggsave(p_pool_qc, filename = paste0(outdir,"Pool_QC_metrics.eps"), width = 12, height = 12, units = "cm")
...
scale_y <- list(
  percent.mt = scale_y_continuous(limits = c(0, 35)),
  nCount_RNA = scale_y_continuous(limits = c(0, 50000)),
  nFeature_RNA = scale_y_continuous(limits = c(0, 5200))
)
...
p_singlets_qc <- ggplot(QC_metrics_long, aes(factor(Singlets, levels = c(seq(0,15), "all")), QC_Metric_Value)) +
  ...
  geom_text(data = N_dt, aes(y = 40000, label=N, hjust=0,vjust=0,angle=45))
...
p_doublets_qc <- ggplot(QC_metrics_long, aes(factor(Doublets, levels = c(seq(0,15), "all")), QC_Metric_Value)) +
  ...
  theme(strip.background = element_blank(),
        strip.placement = "outside") +
  force_panelsizes(rows = c(1.5, 1, 1)) +
  geom_text(data = N_doub_dt, aes(y = 40000, label=N, hjust=0,vjust=0,angle=45))

ggsave(p_doublets_qc, filename = paste0(outdir,"Doublets_v_QC.pdf"), width = 10, height = 8, units = "cm")
ggsave(p_doublets_qc, filename = paste0(outdir,"Doublets_v_QC.png"), width = 10, height = 8, units = "cm")
...
p_demultiplexing_singlets_qc <- ggplot(QC_metrics_long, aes(factor(Demultiplexing_Singlets, c(seq(0,8), "all")), QC_Metric_Value)) +
  ...
  theme(strip.background = element_blank(),
        strip.placement = "outside") +
  force_panelsizes(rows = c(1.5, 1, 1)) +
  geom_text(data = N_demux_dt, aes(y = 40000, label=N, hjust=0,vjust=0,angle=45))
...
p_demultiplexing_doublets_qc <- ggplot(QC_metrics_long, aes(factor(Demultiplexing_Doublets, seq(0,8)), QC_Metric_Value)) +
  ...
  theme(strip.background = element_blank(),
        strip.placement = "outside") +
  force_panelsizes(rows = c(1.5, 1, 1)) +
  geom_text(data = N_demux_doub_dt, aes(y = 40000, label=N, hjust=0,vjust=0,angle=45))
...
p_doublet_detecting_singlets_qc <- ggplot(QC_metrics_long, aes(factor(DoubletDetecting_Singlets, c(seq(0,7), "all")), QC_Metric_Value)) +
  ...
  theme(strip.background = element_blank(),
        strip.placement = "outside")  +
  force_panelsizes(rows = c(1.5, 1, 1)) +
  geom_text(data = N_dd_dt, aes(y = 40000, label=N, hjust=0,vjust=0,angle=45))
...
p_doublet_detecting_doublets_qc <- ggplot(QC_metrics_long, aes(factor(DoubletDetecting_Doublets, seq(0,7)), QC_Metric_Value)) +
  ...
  theme(strip.background = element_blank(),
        strip.placement = "outside") +
  force_panelsizes(rows = c(1.5, 1, 1)) +
  geom_text(data = N_dd_doub_dt, aes(y = 40000, label=N, hjust=0,vjust=0,angle=45))
...
# ##### Make upsets of singlet and doublet calls
# upset_singlets <- seurat@meta.data[,c("demuxalot_DropletType", "demuxalot_refined_DropletType", "demuxlet_DropletType", "dropulation_DropletType", "freemuxlet_DropletType", "scSplit_DropletType", "souporcell_DropletType", "vireo_DropletType", 
#                               "DoubletDecon_DropletType", "DoubletDetection_DropletType", "DoubletFinder_DropletType", "scDblFinder_DropletType", "scds_DropletType", "scrublet_DropletType", "solo_DropletType",
#                               "nCount_RNA", "nFeature_RNA", "percent.mt", "percent.rb", "Pool")]
# upset_singlets[,c("demuxalot_DropletType", "demuxalot_refined_DropletType", "demuxlet_DropletType", "dropulation_DropletType", "freemuxlet_DropletType", "scSplit_DropletType", "souporcell_DropletType", "vireo_DropletType", 
#                               "DoubletDecon_DropletType", "DoubletDetection_DropletType", "DoubletFinder_DropletType", "scDblFinder_DropletType", "scds_DropletType", "scrublet_DropletType", "solo_DropletType")] <- as.data.frame(1*(upset_singlets[,c("demuxlet_DropletType", "freemuxlet_DropletType", "scSplit_DropletType", "souporcell_DropletType", "vireo_DropletType", 
#                               "DoubletDecon_DropletType", "DoubletDetection_DropletType", "DoubletFinder_DropletType", "scDblFinder_DropletType", "scds_DropletType", "scrublet_DropletType", "solo_DropletType")] == "singlet"))
# colnames(upset_singlets) <- gsub("_DropletType","",colnames(upset_singlets))
...
singlets_dd_long_list <- lapply(results_list_wide, function(x){
  columns <- c("Barcode", grep("_DropletType", colnames(x), value = TRUE), "Singlets", "Demultiplexing_Singlets", "DoubletDetecting_Singlets")
    temp <- data.table(pivot_longer(x[,..columns], cols = grep("_DropletType", colnames(x), value = TRUE), values_to = "DropletType", names_to = "Software"))
    temp <- temp[DropletType == "singlet"]
    temp$Software <- gsub("_DropletType", "", temp$Software) %>%
        ...
        gsub("solo", "Solo", .) %>%
        gsub("souporcell", "Souporcell", .)
    temp <- temp[Software %in% doubletdetecting_names]
    return(temp)
})
...
doublets_long_list <- lapply(results_list_wide, function(x){
  columns <- c("Barcode", grep("_DropletType", colnames(x), value = TRUE), "Doublets", "Demultiplexing_Doublets", "DoubletDetecting_Doublets")
    ...
    temp$Software <- gsub("_DropletType", "", temp$Software) %>%
        ...
        gsub("solo", "Solo", .) %>%
        gsub("souporcell", "Souporcell", .)
      temp$Doublets <- factor(temp$Doublets, levels = 1:15)
    return(temp)
})
...
singlets_temp <- data.table(table(results_wide[Singlets == 15 & MajorityAssignment_N == 8][,c("Pool", "MajorityAssignment_ID")]))[N > 0]
colnames(singlets_temp) <- c("Pool","Individual", "Count")

doublets_temp <- data.table(table(results_wide[Doublets == 15][,c("Pool", "MajorityDropletType")]))
...
individuals <- lapply(unique(singlets_temp$Pool), function(x){
  temp <- fread(paste0(paste0(dir, "/", x, "/popscle/Individuals.txt")), col.names = c("Individual"), header = FALSE)
...
pSingletsBox_doubletBar <- ggplot() +
  ...
  new_scale("fill") +
  geom_boxplot(data = singlets_temp, aes(x=Pool, y = Count, fill = as.factor(N_genotyped), color = "")) +
  scale_color_manual("Singlets Per\nIndividual", values=1) +
  scale_fill_manual(values = c("#E3F1E7", "#74A59A", "#255C65"), name = "Number\nIndividuals\nin Pool") +
  theme_classic() +
...
ggsave(filename = paste0(outdir, "SingletsBoxPlot_DoubletBar_genotyped_test.png"), plot = pSingletsBox_doubletBar, width = 4, height = 3.5)
ggsave(filename = paste0(outdir, "SingletsBoxPlot_DoubletBar_genotyped_test.pdf"), plot = pSingletsBox_doubletBar, width = 4, height = 3.5)


# ### Get the number of singlets per individual average ###
# mean(results_singlets_df$Count) # 503.8861



##### Make a dataframe that has the number of unique inidividuals identified per software per pool #####
Indiv_identified <- data.table(Pool = unique(results_wide$Pool),"demuxalot" = as.numeric(NA), "demuxalot_refined" = as.numeric(NA), "demuxlet" = as.numeric(NA), "dropulation" = as.numeric(NA), "freemuxlet" = as.numeric(NA),"scSplit" = as.numeric(NA),"souporcell" = as.numeric(NA),"vireo" = as.numeric(NA))


for (pool in Indiv_identified$Pool){
  Indiv_identified[which(Indiv_identified$Pool == pool),"demuxalot"] <- length(unique(results_wide[Pool == pool]$demuxlet_Assignment)[!(unique(results_wide[Pool == pool]$demuxlet_Assignment) %in% c("doublet","unsure","unassigned"))])
  Indiv_identified[which(Indiv_identified$Pool == pool),"demuxalot_refined"] <- length(unique(results_wide[Pool == pool]$demuxlet_Assignment)[!(unique(results_wide[Pool == pool]$demuxlet_Assignment) %in% c("doublet","unsure","unassigned"))])
  ...
  Indiv_identified[which(Indiv_identified$Pool == pool),"dropulation"] <- length(unique(results_wide[Pool == pool]$demuxlet_Assignment)[!(unique(results_wide[Pool == pool]$demuxlet_Assignment) %in% c("doublet","unsure","unassigned"))])
...
Indiv_identified_long <- pivot_longer(Indiv_identified,cols = c("demuxalot", "demuxalot_refined", "demuxlet", "dropulation", "freemuxlet","scSplit","souporcell","vireo","Genotyped"), names_to = "Software")
Indiv_identified_long$Software <- factor(Indiv_identified_long$Software, levels = c("demuxalot", "demuxalot_refined", "demuxlet", "dropulation", "freemuxlet","scSplit","souporcell","vireo","Genotyped"))
...
Indiv_identified_percent$demuxalot_percentage <- Indiv_identified$demuxlet/Indiv_identified$Genotyped
Indiv_identified_percent$demuxalot_refined_percentage <- Indiv_identified$demuxlet/Indiv_identified$Genotyped
Indiv_identified_percent$demuxlet_percentage <- Indiv_identified$demuxlet/Indiv_identified$Genotyped
Indiv_identified_percent$dropulation_percentage <- Indiv_identified$demuxlet/Indiv_identified$Genotyped
...
Indiv_identified_percent$demuxalot <- NULL
Indiv_identified_percent$demuxalot_refined <- NULL
Indiv_identified_percent$demuxlet <- NULL
Indiv_identified_percent$dropulation <- NULL
...
Indiv_identified_percent_long <- pivot_longer(Indiv_identified_percent,cols = c("demuxalot_percentage","demuxalot_refined_percentage","demuxlet_percentage","dropulation_percentage","freemuxlet_percentage","scSplit_percentage","souporcell_percentage","vireo_percentage"), names_to = "Software")
Indiv_identified_percent_long$Software <- factor(Indiv_identified_percent_long$Software, levels = c("demuxalot_percentage","demuxalot_refined_percentage","demuxlet_percentage","dropulation_percentage","freemuxlet_percentage","scSplit_percentage","souporcell_percentage","vireo_percentage"))
Indiv_identified_percent_long$Pool  <- gsub("OneK1K_scRNA_Sample","Pool", Indiv_identified_percent_long$Pool)
...
Indiv_missed$Demuxalot_percentage <- Indiv_identified$Genotyped- Indiv_identified$demuxlet
Indiv_missed$`Demuxalot (refined)_percentage` <- Indiv_identified$Genotyped- Indiv_identified$demuxlet
Indiv_missed$Demuxlet_percentage <- Indiv_identified$Genotyped- Indiv_identified$demuxlet
Indiv_missed$Dropulation_percentage <- Indiv_identified$Genotyped- Indiv_identified$demuxlet
...
colors <- list("Number Individuals in Pool" = c("#E3F1E7", "#74A59A", "#255C65"))
...
pdf(paste0(outdir, "NumberIndividualsMissed_heatmap.pdf"), width = 7, height = 2.5)
...
heatmap_plot <- ggplot(pct_agree_summary_combined, aes(Software_1, Software_2, fill=mean)) +
	...
	theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust=0.5),
		plot.title = element_text(hjust = 0.5)) +
	scale_fill_gradient(low="#FBFEF9",high="#A63446", limits=c(0.7,1)) 
